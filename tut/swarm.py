...
reviewer = Agent(name="reviewer", system_prompt="You are a code review specialist...")
architect = Agent(name="architect", system_prompt="You are a system architecture specialist...")
report_writer = Agent(name="report_writer", system_prompt="You are a report writing expert, give brief summary below 500 words adressing user input")
...
# Create a swarm with these agents
swarm = Swarm(
    [researcher_agent, coder, reviewer, architect, report_writer],
    max_handoffs=20,
    max_iterations=20,
    execution_timeout=900.0,  # 15 minutes
    node_timeout=300.0,       # 5 minutes per agent
...
@tool
def run_swarm_task(task: str) -> str:
    """Run the swarm on a given task and return the result."""
    result = swarm(task)
    print(f"Status: {result.status}")
    print(f"Node history: {[node.node_id for node in result.node_history]}")
    return str(result)
